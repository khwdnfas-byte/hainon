// dashboard.js (1/2) — العمليات المالية: Dashboard، إضافة/تعديل/حذف، أرشفة، ديون، تقارير
import { auth, db } from './firebase-config.js';
import {
  collection, addDoc, doc, getDoc, getDocs, query, where, orderBy,
  onSnapshot, serverTimestamp, updateDoc, deleteDoc, limit, Timestamp
...
export let state = {}; // سيُشار إليه من app.js
...
// ---------- حساب الأرباح والخسائر (متوسط التكلفة المرجح، دقيق) ----------
...
export function loadDashboardPage() {
  ...
  onSnapshot(q, (snapshot) => {
    ...
    const stats = [
      ...
      { icon: 'fa-undo-alt', label: 'مرتجع', type: 'returned', usd: 0, syp: 0 }
    ];
    ...
    const profit = (vipLevel >= 2) ? calculateProfitLossAccurate(allTx, 'USD') : null;
    ...
    section.innerHTML = `
        ...
        ${stats.map(s => `
          <div class="stat-card stat-net ${vipClass}" data-type="${s.type}">
            ...
            <div class="stat-value">
              <div>${formatCurrency(s.usd)}</div>
              <div><small>${formatCurrency(s.syp, 'SYP')}</small></div>
            </div>
      ...
      <!-- نموذج إضافة العملية وجدول اليوم -->
    ...
    if (todayTx.length > 0) {
      ...
      todayTx.forEach(t => {
        ...
        row.innerHTML = `
          ...
          <td>${t.currency}</td>
          <td>${formatDateEn(createdDate)}</td>
          <td>${formatTimeEn(createdDate)}</td>
...
export function loadReportsPage() {
  ...
  getDocs(qFirst).then(snap => {
    if (!snap.empty) {
      ...
      if (firstDate) $('#report-from-date').min = firstDate.toISOString().split('T')[0];
    }
  });

  $('#report-type-select').addEventListener('change', function() {
    const toggles = $('#report-type-toggles');
    if (toggles) toggles.style.display = this.value === 'all' ? 'flex' : 'none';
  });
...
function generateReport() {
  ...
  onSnapshot(q, (snapshot) => {
    ...
    const profit = (vipLevel >= 2) ? calculateProfitLossAccurate(filtered, 'USD') : null;
...
}// dashboard.js (2/2) — نظام VIP، المحادثات، خدمة العملاء، إدارة المستخدمين، الإعدادات
// ... تابع الملف dashboard.js بعد نهاية الجزء الأول
...
/**
 * إنشاء طلب ترقية VIP بعد تأكيد الدفع
 */
export async function createVipRequest(level, operationNumber) {
  ...
  await addDoc(collection(db, 'vipRequests'), {
    uid: user.uid,
    name: userDataSnap.name,
    email: userDataSnap.email,
    serialId: userDataSnap.serialId,
    level: level,
...
/**
 * عرض طلبات VIP في صفحة إدارة المستخدمين (للأدمن والمشرفين)
 */
...
export function loadVipPaymentPage() {
  ...
  const timerInterval = setInterval(() => {
    ...
    $('#payment-timer').textContent = `${String(mins).padStart(2, '0')}:${String(secs).padStart(2, '0')}`;
    if (timeLeft <= 0) {
...
// ========== محادثة الإدارة الداخلية (للأدمن والمشرفين) ==========
...
export function loadVipSupportChat() {
  ...
  section.innerHTML = `
    ...
    <div class="chat-container" style="height:calc(100vh - 280px);">
      <div class="chat-messages" id="vip-support-messages">
        <p style="text-align:center;color:var(--text-muted);">⏳ جاري تحميل المحادثة...</p>
      </div>
      <div class="chat-input-area">
  ...
  onSnapshot(q, (snapshot) => {
    messagesDiv.innerHTML = '';
    if (snapshot.empty) {
      messagesDiv.innerHTML = '<p style="text-align:center;color:var(--text-muted);">أهلاً بك! كيف يمكننا مساعدتك؟</p>';
    }
    snapshot.forEach(docSnap => {
      const msg = docSnap.data();
  ...
  const sendFunc = async () => {
    ...
    try {
      ...
      $('#vip-support-input').value = '';
    } catch (e) {
      showToast('فشل في الإرسال', 'error');
    }
  };

  $('#vip-support-send')?.addEventListener('click', sendFunc);
  $('#vip-support-input')?.addEventListener('keypress', e => { if (e.key === 'Enter') sendFunc(); });
}

// ========== صفحة إدارة المستخدمين (للأدمن والمشرفين) ==========
export async function loadUsersPage() {
  ...
  usersSnapshot.forEach(docSnap => {
    ...
    const isOnline = u.lastLogin?.toDate() > new Date(Date.now() - 5 * 60 * 1000); // مبسط
    const lastSeen = u.lastLogin ? formatDateTimeEn(u.lastLogin.toDate()) : '---';
    const deviceInfo = u.device || {};
    ...
    const locationHtml = u.location?.city ? `<a href="${locationLink}" target="_blank" style="color:var(--gold);text-decoration:underline;">${locationText}</a>` : '---';
    const createdAt = u.createdAt ? formatDateEn(u.createdAt.toDate()) : '---';
    ...
    tbody.innerHTML += `
      <tr>
        <td><img src="${avatarUrl}" style="width:30px;height:30px;border-radius:50%;border:2px solid var(--gold);"></td>
        <td>${u.name || '---'}</td>
        <td>${u.serialId || '---'}</td>
        <td>${u.email || '---'}</td>
        <td>${roleBadge}</td>
  ...
  // ... (باقي الأحداث مثل تعديل ID، حذف، حظر، إلخ. يمكن إضافتها لاحقاً)
}
...
export function loadSettingsPage() {
  ...
  section.innerHTML = `
    ...
    <div style="max-width:500px;margin:0 auto;">
      ...
      <button id="save-profile-btn" class="btn-primary" style="width:100%;"><i class="fas fa-save"></i> حفظ التعديلات</button>
    </div>
  `;
