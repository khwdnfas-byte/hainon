...
export function loadSiteManagementPage() {
  ...
  if (!section) return;
  if (!isAdmin && !isMod && !isSuperMod) { section.innerHTML = '<div style="text-align:center;padding:40px;color:var(--text-muted);">غير مصرح</div>'; return; }
...
async function searchBars(qt) {
  ...
  if (mu.length === 0) { res.innerHTML = '<p style="color:var(--text-muted);text-align:center;padding:16px;">لا توجد نتائج</p>'; return; }
  res.innerHTML = '<h4 style="margin:12px 0 8px;color:var(--gold);">المستخدمين المطابقين:</h4>';
  for (const u of mu) {
...
export function loadAdminChat() {
  ...
  s.innerHTML = `<div class="chat-container" style="height:calc(100vh - 180px);"><div class="chat-messages" id="acm"><p style="text-align:center;color:var(--text-muted);">⏳ جاري تحميل المحادثة...</p></div><div class="chat-input-area"><input type="text" id="aci" placeholder="اكتب رسالتك..."><button id="acs"><i class="fas fa-paper-plane"></i></button></div></div>`;
  const md = $('#acm');
...
export function loadVipSupportChat() {
  ...
  if (isAdmin || isMod || isSuperMod) {
    s.innerHTML = `<div id="vcl" style="margin-bottom:16px;">⏳ جاري تحميل جهات الاتصال...</div><div id="vca" class="hidden"></div>`;
    loadVipContacts(); return;
  }
  s.innerHTML = `<div class="chat-container" style="height:calc(100vh - 180px);"><div class="chat-messages" id="vsm"><p style="text-align:center;color:var(--text-muted);">⏳ جاري التحميل...</p></div><div class="chat-input-area"><input type="text" id="vsi" placeholder="اكتب رسالتك..."><button id="vss"><i class="fas fa-paper-plane"></i></button></div></div>`;
...
export function loadVipRequestsAdmin() {
  ...
  panel.innerHTML = '<p style="text-align:center;color:var(--text-muted);">⏳ جاري تحميل الطلبات...</p>';
...
export function loadVipRequestSettingsPage() {
  ...
  if (!section) return;
  if (!isAdmin) { section.innerHTML = '<div style="text-align:center;padding:40px;color:var(--text-muted);">غير مصرح</div>'; return; }
  ...
  section.innerHTML = `
    <div class="profile-page">
      <div class="profile-cover">
        ...
        ${canEditCover ? `<div class="profile-actions-top"><button class="btn-outline edit-cover-btn"><i class="fas fa-pen"></i></button></div>` : ''}
        <div class="profile-avatar-large ${getVipAvatarClass(u.role)}" id="profile-avatar-img">
          <img src="${u.avatar || 'https://ui-avatars.com/api/?name='+encodeURIComponent(u.name||'?')+'&background=D4AF37&color=111&size=200'}">
        </div>
      </div>
      <div class="profile-info">
        ...
        <div style="margin-top:12px;padding:8px;background:rgba(212,175,55,0.1);border-radius:8px;font-size:12px;color:var(--gold);">${getVipBadgeText(u.role) || 'مستخدم عادي'}</div>
      </div>
    </div>`;
...
export function loadSettingsPage() {
  ...
  section.innerHTML = `
    <div style="max-width:500px;margin:0 auto;">
      ...
      <div class="input-group" style="margin-bottom:12px;"><label><i class="fas fa-building"></i> اسم الشركة</label><input type="text" id="settings-company" value="${userData?.company||''}" placeholder="اسم شركتك"></div>
  ...
  $('#save-profile-btn')?.addEventListener('click', async () => {
    ...
    const c = $('#settings-company')?.value.trim();
    if (!n) return showToast('الاسم مطلوب','error');
    await updateDoc(doc(db,'users',auth.currentUser.uid),{name:n,bio:b,company:c||''});
    userData.name=n; userData.bio=b; userData.company=c||'';
...
export async function loadVipUsersList() {
  ...
  let html = '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(250px,1fr));gap:12px;margin-top:8px;">';
