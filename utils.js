...
// ---------- نظام مستوى الحساب (LV) اللانهائي ----------
export function calculateLevel(points) {
  ...
  while (points >= required) {
    level++;
    required = 2500 * Math.pow(2, level - 1);
  }
  return level - 1;
}

export function getLevelInfo(level) {
  const nextRequired = level === 0 ? 2500 : 2500 * Math.pow(2, level);
  const colors = [
    'var(--text-muted)',   // LV 0 - رمادي
    'var(--gold)',         // LV 1 - ذهبي
    'var(--vip2-color)',   // LV 2 - أخضر
    'var(--vip3-color)',   // LV 3 - بنفسجي
    'var(--vip4-color)',   // LV 4 - أزرق
    'var(--admin-color)',  // LV 5 - برتقالي
    '#FFD700',            // LV 6 - ذهبي لامع
    '#FF4500',            // LV 7 - أحمر ناري
    '#00FFFF',            // LV 8 - سماوي
    '#FF00FF',            // LV 9 - وردي
  ];
  const color = colors[Math.min(level, colors.length - 1)] || '#FFFFFF';
  return {
    name: `LV ${level}`,
    nextRequirement: nextRequired,
    color: color
  };
}
