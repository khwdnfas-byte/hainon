...
export function loadTransactionsPage(){
  ...
  onSnapshot(query(collection(db,'archives'),where('uid','==',currentUser.uid),orderBy('date','desc')),(sn)=>{
    ...
    l.innerHTML=''; sn.forEach(d=>{const a=d.data(); const dv=document.createElement('div'); dv.className='stat-card'; dv.style.cssText='cursor:pointer;margin-bottom:8px;';
      ...
      dv.addEventListener('click',()=>showArchiveDetail(a, d.id, ft)); l.appendChild(dv);});
  });
...
function showArchiveDetail(archive, archiveId, filterType) {
  ...
  detail.innerHTML = `
    ...
    <tbody>${txs.length === 0 ? '<tr><td colspan="7">لا توجد عمليات</td></tr>' : txs.map((t, idx) => `
      <tr>
        ...
        <td><button class="btn-outline btn-sm edit-archive-tx-btn" data-archive="${archiveId}" data-index="${idx}"><i class="fas fa-edit"></i></button></td>
        <td><button class="btn-outline btn-sm delete-archive-tx-btn" data-archive="${archiveId}" data-index="${idx}" style="color:var(--red);border-color:var(--red);"><i class="fas fa-trash"></i></button></td>
      </tr>`).join('')}</tbody></table></div>
  `;
  $('#back-to-archives')?.addEventListener('click', () => { detail.classList.add('hidden'); list.classList.remove('hidden'); });

  detail.querySelectorAll('.edit-archive-tx-btn').forEach(btn => {
    btn.addEventListener('click', () => editArchiveTransaction(btn.dataset.archive, parseInt(btn.dataset.index)));
  });
  detail.querySelectorAll('.delete-archive-tx-btn').forEach(btn => {
    btn.addEventListener('click', () => deleteArchiveTransaction(btn.dataset.archive, parseInt(btn.dataset.index)));
  });
}

async function editArchiveTransaction(archiveId, index) {
  const snap = await getDoc(doc(db, 'archives', archiveId));
  ...
  txs[index].amount = parseFloat(newAmount);
  await updateDoc(doc(db, 'archives', archiveId), { transactions: txs });
  showToast('تم تعديل العملية', 'success');
  showArchiveDetail({ ...archive, transactions: txs }, archiveId);
}

async function deleteArchiveTransaction(archiveId, index) {
  if (!(await showConfirm('حذف هذه العملية من الأرشيف؟'))) return;
  const snap = await getDoc(doc(db, 'archives', archiveId));
  if (!snap.exists()) return;
  const archive = snap.data();
  const txs = archive.transactions || [];
...
export function loadDebtsPage(){
  ...
  ab.addEventListener('click',()=>{
    const dt=$('#dd'); if(!dt||!l) return; l.classList.add('hidden');dt.classList.remove('hidden');
    ...
    onSnapshot(query(collection(db,'transactions'),where('uid','==',currentUser.uid),orderBy('createdAt','desc')),(sn)=>{
      const ts=[]; sn.forEach(d=>ts.push(d.data()));
      const ds=ts.filter(t=>['debt_in','debt_out','debt_received','debt_paid'].includes(t.type));
      const tb=[
        {ti:'<i class="fas fa-hand-holding-usd"></i> دين لنا',ty:'debt_in'},
        {ti:'<i class="fas fa-hand-holding-usd"></i> دين علينا',ty:'debt_out'},
        {ti:'<i class="fas fa-check-circle"></i> دين مقبوض',ty:'debt_received'},
        {ti:'<i class="fas fa-times-circle"></i> دين مدفوع',ty:'debt_paid'}
      ];
      let h=''; tb.forEach(b=>{
        const f=ds.filter(t=>t.type===b.ty);
        h+=`<h4 style="margin:12px 0 8px;color:var(--gold);">${b.ti} (${f.length})</h4>
        <div class="table-container"><table><thead><tr><th>المنتج</th><th>المبلغ</th><th>العملة</th><th>التاريخ</th></tr></thead><tbody>
        ${f.length===0?'<tr><td colspan="4">لا توجد</td></tr>':f.map(t=>`<tr><td>${t.productName||'---'}</td><td>${formatCurrency(t.amount,t.currency)}</td><td>${t.currency}</td><td>${t.createdAt?formatDateEn(t.createdAt.toDate()):''}</td></tr>`).join('')}</tbody></table></div>`;
      });
      const c=$('#adc'); if(c) c.innerHTML=h;
    });
  });
  l.appendChild(ab);
}
