...
// app.js — الإصدار المُحسَّن (Lazy Loading + شريط علوي جديد)
...
function updateTopbarTitle(page) {
  const titles = {
    dashboard: 'الرئيسية', transactions: 'العمليات المؤرشفة',
...
// ========== بناء القائمة الجانبية (بالترتيب الجديد) ==========
export function buildSidebar() {
  ...
  // 1. تفعيل VIP (فقط لغير VIP)
  if (!state.isVip && !state.isAdmin && !state.isSuperMod && !state.isMod) {
  ...
  // 7-9. الإدارة (للأدمن والمشرفين فقط)
  ...
  // 13. شريط الكتابة (أسفل القائمة)
  if (state.isVip || state.isAdmin || state.isMod || state.isSuperMod) {
...
function setupEventListeners() {
  ...
  $('#topbar-logo')?.addEventListener('click', async () => { const dm = await getDashboardModule(); dm.viewPublicProfile(auth.currentUser?.uid); });
