...
import {
    ...
    updateDoc, deleteDoc, limit, Timestamp, increment,
...
let bottomBarState = 'home'; // 'home' | 'friendships' | 'groups'
...
function getTypeLabel(type) {
    const labels = {
        ...
        purchase: '🛒 شراء', debtToUs: '🟢 دين لنا', debtOnUs: '🔴 دين علينا',
        debtReceived: '✅ دين مقبوض', debtPaid: '💸 دين مدفوع', returned: '↩️ مرتجع'
    };
...
function calculateProfitLoss(txs, cur = 'USD') {
    ...
    txs.forEach(t => {
        ...
        if (t.type === 'purchase') p.purchase += a;
            ...
            p.returned += a;
            // حساب الربح المسترجع للقطعة المرتجعة
...
function setupPresence(uid) {
    const upr = presenceRef(uid);
    const cr = ref(rtdb, '.info/connected');
    onValue(cr, snap => {
        if (snap.val() === true) {
            onDisconnect(upr).set({ status: 'offline', lastSeen: rtdbTimestamp() });
            set(upr, { status: 'online', lastSeen: rtdbTimestamp() });
        }
    });
}
...
// ---------- دالة إرسال رمز تأكيد عبر EmailJS ----------
...
async function handleRegister(e) {
    ...
    try {
        ...
        pendingCodes[email] = { code, expires: Date.now() + 10 * 60 * 1000 }; // صالح لمدة 10 دقائق
...
async function handleLogin(e) {
    ...
    try {
        ...
        // التحقق من الحظر
        if (userData.blocked) {
            const blockReason = userData.blockReason || 'غير محدد';
            const blockExpiry = userData.blockExpiry ? new Date(userData.blockExpiry.toDate()).toLocaleString('ar-SY') : 'دائم';
            await signOut(auth);
            hideLoading();
            showToast(`🚫 حسابك محظور. السبب: ${blockReason}. ينتهي: ${blockExpiry}`, 'error');
            return;
        }
...
async function handleForgotPassword() {
    ...
    try {
        ...
        // الانتقال إلى مودال إعادة التعيين
...
async function handleResetPassword() {
    ...
    const pending = pendingCodes[email];
    if (!pending || pending.type !== 'reset') return showToast('انتهت صلاحية الرمز', 'error');
    
    // هنا لا نستطيع تغيير كلمة مرور المستخدم دون تسجيل دخول، لذا نستخدم sendPasswordResetEmail كبديل
    // لكن للتبسيط نطلب من المستخدم استخدام الرابط، أو نستخدم Admin SDK (غير متاح هنا)
    // لذا سنوجه المستخدم: نرسل رابط إعادة تعيين بدلاً من الرمز
    try {
        ...
        showToast('❌ فشل في إرسال رابط إعادة التعيين', 'error');
    }
}
...
function loadDashboardPage() {
    ...
    onSnapshot(q, (snapshot) => {
        ...
        // حدث تغيير نوع العملية (إظهار/إخفاء الكمية وتغيير placeholder)
...
let currentHistoryIndex = {}; // { transId: currentIndex }
...
// ---------- تصفية العمليات حسب النوع (للتنقل من البطاقات) ----------
function filterTransactionsByType(type) {
    // هذه الدالة تستخدم في صفحة العمليات
    sessionStorage.setItem('filterType', type);
}

// ---------- تحديث عنوان الشريط العلوي حسب الصفحة ----------
...
function loadTransactionsPage() {
    ...
    onSnapshot(q, (snapshot) => {
        ...
        // تطبيق التصفية إذا تم التنقل من بطاقة
        if (filterType && snapshot.size > 0) {
            // فتح أول أرشيف وتصفيته
        }
    });
}
...
function loadReportsPage() {
    ...
    // تصدير PDF (مؤقتاً وظيفة تنبيهية)
...
function setupReportDates() {
    // جلب أقدم وأحدث عملية لتحديد النطاق
    const q = query(
        collection(db, 'transactions'),
        where('uid', '==', currentUser.uid),
...
function generateReport() {
    ...
    onSnapshot(q, (snapshot) => {
        ...
        if (vipLevel >= 3 || isAdmin || isSuperMod) {
            if (!noWatermark) {
                header += `<div style="text-align:center;opacity:0.1;font-size:48px;position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);z-index:0;">HAINON</div>`;
            }
        } else if (vipLevel === 2) {
            // VIP2: يختفي التوقيع والعلامة المائية، يظهر اسم المستخدم في الوسط
            header += `<div style="text-align:center;font-weight:700;font-size:24px;color:var(--gold);">${userData.name}</div>`;
        } else if (vipLevel === 1) {
            // VIP1: تختفي العلامة المائية فقط، يبقى التوقيع HAINON
            ...
            // المستخدم العادي: العلامة المائية وتوقيع HAINON
        ...
        // صورة المستخدم (إذا لم تكن VIP3 أزالتها)
        if (!(vipLevel >= 3 && !noWatermark && false)) { // حسب القواعد VIP3 قد تخفيها
...
function loadFriendshipsPage() {
    ...
    section.innerHTML = `
        ...
        <!-- قائمة المستخدمين -->
        <div id="friends-list" class="friends-grid" style="display:grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap:12px;">
...
function loadChatPage() {
    ...
    if (isAdmin || isMod || isSuperMod) {
        ...
        let currentFilter = 'support'; // 'support' or 'reports'
...
function loadAdminContacts(filter = 'support') {
    ...
    if (filter === 'support') {
        // جلب المستخدمين الذين راسلوا خدمة العملاء
...
async function loadPrivacyPage() {
    ...
    // جلب المحتوى من Firestore (يمكن للأدمن تعديله)
...
function setupWriteBar() {
    ...
    $('#write-bar-send')?.addEventListener('click', async () => {
        ...
        // فلترة الكلمات المسيئة (بسيطة)
        const badWords = ['كلمة1', 'كلمة2']; // استبدل بقائمة حقيقية
        if (badWords.some(w => text.includes(w))) {
            return showToast('⚠️ المحتوى غير لائق', 'error');
        }
...
function updateVipTopBar() {
    ...
    onSnapshot(q, (snapshot) => {
        ...
        showNext();
        setInterval(showNext, 15000); // تغيير كل 15 ثانية
    });
}
...
function navigateTo(page) {
    ...
    switch (page) {
        ...
        case 'activate-vip': navigateTo('chat'); break; // يوجه لخدمة العملاء لطلب VIP
    }
...
function updateBottomBar(page) {
    ...
    if (page === 'dashboard') {
        ...
        btnFriends.textContent = '👥 تكوين صداقات';
        btnGroups.textContent = '💬 المجموعات';
        ...
        btnGroups.classList.add('hidden');
        btnBack.classList.remove('hidden');
        btnBack.textContent = '🏠 العودة إلى الرئيسية';
    }
}
...
function toggleSidebar() {
    ...
    // إخفاء الصورة في الشريط العلوي عند فتح القائمة
...
function setupEventListeners() {
    ...
    // الشريط السفلي
    $('#bottom-btn-friendships')?.addEventListener('click', () => {
        friendshipPageActive = true;
        groupsPageActive = false;
        navigateTo('friendships');
    });
    $('#bottom-btn-groups')?.addEventListener('click', () => {
        groupsPageActive = true;
        friendshipPageActive = false;
        navigateTo('groups');
    });
    $('#bottom-btn-back')?.addEventListener('click', () => {
        friendshipPageActive = false;
        groupsPageActive = false;
        navigateTo('dashboard');
    });
...
// =============================================
// مراقب حالة المصادقة وبدء التطبيق
// =============================================
