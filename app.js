...
async function sendEmailCode(email, code) {
    ...
    try {
        ...
        console.log('✅ تم إرسال البريد:', response);
        return true;
    } catch (error) {
...
async function handleRegister(e) {
    ...
    try {
        ...
        if (sent) {
            ...
            showToast('⚠️ تم إنشاء الحساب لكن فشل إرسال رمز التأكيد. حاول إعادة الإرسال لاحقاً.', 'error');
        }
...
async function loadDashboardPage() {
    // ** تمت الإضافة: تفعيل الأرشفة التلقائية **
...
function loadSupportChat() {
    ...
    const sendFunc = async () => {
        ...
        try {
            ...
            // ** تمت الإضافة: الرد التلقائي بعد أول رسالة **
            const q = query(collection(db, 'supportMessages'), orderBy('createdAt', 'asc'));
            const snapshot = await getDocs(q);
            let count = 0;
...
function assignVipModal(uid, currentRole) {
    ...
    updateDoc(doc(db, 'users', uid), {
        ...
        showVipConfetti(message); // ** تمت الإضافة **
        loadUsersPage();
    });
}
...
function showVipConfetti(message = 'مبروك! تمت ترقيتك إلى VIP') {
    ...
    box.innerHTML = `<div style="font-size:48px;">🎉</div><h2 style="color:var(--gold);margin:16px 0;">تهانينا!</h2><p style="color:var(--text-primary);font-size:16px;">${message}</p><button id="vip-confetti-close" class="btn-primary" style="margin-top:20px;">شكراً</button>`;
...
function showImageReviewModal(approvalId, data) {
    ...
    modal.innerHTML = `
        ...
        <div class="modal-buttons">
            <button class="btn-primary approve-btn">✅ قبول</button>
...
onAuthStateChanged(auth, async (user) => {
    ...
    if (user) {
        ...
        if (exists) {
            ...
            if (isAdmin || isMod || isSuperMod) {
                monitorPresence();
                listenForImageApprovals(); // ** تمت الإضافة **
            }
            updateVipTopBar();
