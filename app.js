...
function buildSidebar() {
    ...
    addBtn('chat', 'fa-comments', isAdmin || isMod || isSuperMod ? 'مشاكل المستخدمين' : 'خدمة العملاء');
        ...
        <div class="form-row">
            <div class="input-group">
                ...
                <select id="report-type-select">
                    ...
                    <option value="incoming"><i class="fas fa-download"></i> وارد</option>
                    <option value="outgoing"><i class="fas fa-upload"></i> صادر</option>
                    <option value="sale"><i class="fas fa-tag"></i> بيع</option>
                    <option value="purchase"><i class="fas fa-shopping-cart"></i> شراء</option>
                    <option value="debt_in"><i class="fas fa-hand-holding-usd"></i> دين لنا</option>
                    <option value="debt_out"><i class="fas fa-hand-holding-usd"></i> دين علينا</option>
                    <option value="debt_received"><i class="fas fa-check-circle"></i> دين مقبوض</option>
                    <option value="debt_paid"><i class="fas fa-times-circle"></i> دين مدفوع</option>
                    <option value="returned"><i class="fas fa-undo-alt"></i> مرتجع</option>
                </select>
            </div>
        </div>
...
function loadFriendshipsPage() {
    ...
    section.innerHTML = `
        <h2><i class="fas fa-user-friends"></i> تكوين صداقات</h2>
        ...
        <!-- قائمة الأصدقاء -->
        <h3 style="margin:20px 0 12px;"><i class="fas fa-users"></i> أصدقائي (<span id="friends-count">0</span>/${getMaxFriends()})</h3>
    ...
    filtered.forEach(u => {
        ...
        card.innerHTML = `
            ...
            <div style="display:flex;gap:6px;margin-top:8px;flex-wrap:wrap;">
                <button class="btn-outline btn-sm send-friend-btn" data-uid="${u.uid}" data-name="${u.name}"><i class="fas fa-user-plus"></i> إضافة صديق</button>
                <button class="btn-outline btn-sm view-profile-btn" data-uid="${u.uid}" style="font-size:11px;"><i class="fas fa-id-card"></i> ملف</button>
                <button class="btn-outline btn-sm report-user-btn" data-uid="${u.uid}" style="font-size:11px;color:var(--red);border-color:var(--red);"><i class="fas fa-flag"></i> بلاغ</button>
            </div>
        `;
        ...
        card.innerHTML = `
            <div style="display:flex;align-items:center;gap:8px;">
                ...
                <div style="flex:1;min-width:0;">
                    ...
                    <div style="font-size:10px;color:var(--text-muted);">${isOnline ? '<i class="fas fa-circle" style="color:var(--green);"></i> متصل' : '<i class="fas fa-circle" style="color:var(--red);"></i> غير متصل'}</div>
                </div>
                <div style="display:flex;gap:4px;">
                    <button class="btn-outline btn-sm chat-friend-btn" data-uid="${u.uid}" title="مراسلة"><i class="fas fa-comment-dots"></i></button>
                    <button class="btn-outline btn-sm remove-friend-btn" data-uid="${u.uid}" data-name="${u.name}" title="إلغاء الصداقة" style="color:var(--red);border-color:var(--red);"><i class="fas fa-user-slash"></i></button>
                </div>
            </div>
        `;
        list.appendChild(card);
...
function startFriendChat(uid, name) {
    ...
    section.innerHTML = `
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;">
            <button class="btn-outline btn-sm" id="back-to-friends"><i class="fas fa-arrow-right"></i> عودة</button>
    ...
    onSnapshot(q, (snapshot) => {
        ...
        snapshot.forEach(doc => {
            ...
            if ((msg.from === currentUser.uid && msg.to === uid) ||
                ...
                const frameClass = msg.from === currentUser.uid ? '' : getMessageFrameClassSync(msg.from);
                messagesDiv.innerHTML += `
                    <div class="chat-msg ${isSent ? 'sent' : 'received'} ${frameClass}">
    ...
    section.innerHTML = `
        <div class="profile-page">
            ...
            <div class="profile-info">
                ...
                <div class="profile-status">${isOnline ? '<i class="fas fa-circle" style="color:var(--green);"></i> متصل الآن' : '<i class="fas fa-circle" style="color:var(--red);"></i> غير متصل'} ${u.lastSeen && u.privacy?.showLastSeen ? '· آخر ظهور: ' + new Date(u.lastSeen.toDate()).toLocaleString('ar-SY') : ''}</div>
                <div class="profile-actions">
                    ${!isFriend ? `<button class="btn-primary btn-sm send-friend-btn" data-uid="${uid}" data-name="${u.name}"><i class="fas fa-user-plus"></i> إضافة صديق</button>` :
                      `<button class="btn-outline btn-sm remove-friend-btn" data-uid="${uid}" data-name="${u.name}"><i class="fas fa-user-slash"></i> إلغاء الصداقة</button>`}
                    <button class="btn-outline btn-sm report-user-btn" data-uid="${uid}" style="color:var(--red);border-color:var(--red);"><i class="fas fa-flag"></i> بلاغ</button>
                    <button class="btn-outline btn-sm block-user-btn" data-uid="${uid}" data-name="${u.name}"><i class="fas fa-ban"></i> حظر</button>
                </div>
            </div>
        </div>
    `;
...
function loadGroupsPage() {
    ...
    section.innerHTML = `
        <h2><i class="fas fa-users"></i> المجموعات</h2>
        <button id="create-group-btn" class="btn-primary" style="margin-bottom:16px;"><i class="fas fa-plus-circle"></i> إنشاء مجموعة</button>
...
async function loadGroups() {
    ...
    onSnapshot(q, (snapshot) => {
        ...
        snapshot.forEach(doc => {
            ...
            card.style.cssText = 'cursor:pointer;';
            card.innerHTML = `
                <div style="font-weight:700;"><i class="fas fa-comments"></i> ${g.name}</div>
                ...
                <button class="btn-outline btn-sm open-group-btn" data-id="${doc.id}" data-name="${g.name}" style="margin-top:6px;"><i class="fas fa-door-open"></i> فتح</button>
            `;
            list.appendChild(card);
        });
...
function openGroupChat(groupId, groupName) {
    ...
    chatArea.innerHTML = `
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;">
            <button class="btn-outline btn-sm" id="back-to-groups"><i class="fas fa-arrow-right"></i> عودة</button>
            <h3 style="margin:0;"><i class="fas fa-comments"></i> ${groupName}</h3>
        </div>
        <div class="chat-container" style="height:calc(100vh - 300px);">
            ...
            <div class="chat-input-area">
                <input type="text" id="group-chat-input" placeholder="اكتب رسالتك...">
...
// ---------- تحديد إطار المحادثة حسب مستوى المستخدم ----------
async function getMessageFrameClass(uid) {
    if (uid === currentUser.uid) return '';
    
    const snap = await getDoc(doc(db, 'users', uid));
    if (!snap.exists()) return 'frame-default';
    
    const u = snap.data();
    if (u.role === 'admin') return 'frame-admin';
    if (u.role === 'super_mod') return 'frame-mod';
    if (u.role === 'moderator') return 'frame-mod';
    if (u.role === 'vip3') return 'frame-vip3';
    if (u.role === 'vip2') return 'frame-vip2';
    if (u.role === 'vip1') return 'frame-vip1';
    return 'frame-default';
}

function getMessageFrameClassSync(uid) {
    return 'frame-default';
    }// =============================================
    ...
    onSnapshot(q, (snapshot) => {
        ...
        snapshot.forEach(doc => {
            ...
            if (type === 'support' && (msg.uid === uid || msg.targetUid === uid)) {
                ...
                const frameClass = msg.uid === currentUser.uid ? '' : getMessageFrameClassSync(msg.uid);
                messagesDiv.innerHTML += `
                    <div class="chat-msg ${isSent ? 'sent' : 'received'} ${frameClass}">
                ...
                messagesDiv.innerHTML += `
                    <div class="chat-msg received frame-default">
                        <strong>${msg.fromName || 'مستخدم'}</strong>
                        <p>بلاغ: ${escapeHtml(msg.reason)}</p>
                        <small>${msg.createdAt ? new Date(msg.createdAt.toDate()).toLocaleTimeString('ar-SY', {hour:'2-digit', minute:'2-digit'}) : ''}</small>
                    </div>
                `;
            }
...
function loadSettingsPage() {
    ...
    section.innerHTML = `
        ...
        <div style="max-width:500px;margin:0 auto;">
            ...
            <!-- لون الخط -->
            <h3 style="margin:20px 0 12px;color:var(--gold);"><i class="fas fa-palette"></i> لون الخط</h3>
            <div class="input-group" style="margin-bottom:12px;">
                <label>اختر لون خط واجهتك</label>
                <input type="color" id="settings-text-color" value="${userData?.textColor || '#F5F5F5'}" style="width:100%;height:40px;border:none;background:transparent;cursor:pointer;">
            </div>
...
function setupSettingsEvents() {
    ...
    $('#settings-avatar-upload')?.addEventListener('change', async (e) => {
        ...
        reader.onload = async (ev) => {
            ...
            notifyAdminsAboutImage(ev.target.result, currentUser.uid);
        };
    ...
    $('#save-profile-btn')?.addEventListener('click', async () => {
        ...
        const textColor = $('#settings-text-color').value;
        if (!name) return showToast('الاسم مطلوب', 'error');
        
        const updates = { name, bio, textColor };
...
// =============================================
// تأثير رقاقات التهنئة الذهبية (VIP)
// =============================================
...
// =============================================
// نظام مراجعة الصور (للأدمن والمشرفين)
// =============================================
...
function showImageReviewModal(approvalId, data) {
    ...
    modal.querySelector('.reject-btn').addEventListener('click', async () => {
        await updateDoc(doc(db, 'users', data.uploaderUid), { avatar: `https://ui-avatars.com/api/?name=${encodeURIComponent(data.uploaderName)}&background=D4AF37&color=111&size=200` });
    ...
    if (user) {
        ...
        if (exists) {
            ...
            if (!userData.onboardingCompleted) {
                ...
                $('#app').classList.remove('hidden');
                updateUI();
                applyTextColor(userData.textColor || '#F5F5F5');
                applySavedTheme();
                navigateTo('dashboard');
            }
        } else {
