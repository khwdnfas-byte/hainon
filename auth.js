...
export async function handleRegister(e) {
  ...
  try {
    ...
    await setDoc(doc(db, 'users', user.uid), {
      uid: user.uid, name, email, serialId, company,
      role: 'user', avatar: avatarUrl,
      onboardingCompleted: false, emailVerified: false,
      createdAt: serverTimestamp(), lastLogin: serverTimestamp(),
      bio: '', coverPhoto: '',
      privacy: { whoCanSeeProfile: 'everyone', showStatus: false, showLastSeen: false },
      transactionCount: 0, accountLevel: 0,
      showVipBar: true,
      lastWriteBarTime: null,
      currentBarId: null
    });
...
export async function verifyEmailCode() {
  ...
  if (!code || code.length !== 6) return showToast('أدخل رمزاً مكوناً من 6 أرقام', 'error');
...
export async function updateShowVipBar(show) {
  await updateDoc(doc(db, 'users', auth.currentUser.uid), { showVipBar: show });
}